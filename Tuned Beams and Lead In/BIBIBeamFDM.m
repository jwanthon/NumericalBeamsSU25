...
gamma       = 15;        % Flexural rigidity [N·m²]
L           = 1;        % Beam length [m]
n           = 100;      % Mesh size
...
%% Time-Domain Solution (Euler Method)
...
initDisp = 10;           % Initial displacement at beam center [m]
timeMax  = 10;          % End point of simulation
timestep = .01;        % Timestep

vec_IC = BIBIBeamICs(n, L, gamma, initDisp);
scaledFDM = FDM * gamma / rho;

% Solve for solution matrix using Euler approx
solution = zeros(n,round(timeMax/timestep+1));
solution(:, 1) = vec_IC;
vec_uD = zeros(n,1);
vec_uDD = scaledFDM * solution(:,1);
for i = 2:(timeMax/timestep+1)
    solution(:,i) = solution(:,i-1)+timestep*vec_uD;
    vec_uD = timestep * vec_uDD;
    vec_uDD = scaledFDM * solution(:, i);
end
fprintf('%.2f-second Euler solution generated with timestep %f\n', timeMax, timestep);
